...
\section{Пример 1: система второго порядка}
...
Требуется минимизировать величину $\sum_{t=0}^{T-1}\norm{u(t)}^2$ при гарантированном соблюдении ограничений на значения выходных сигналов в последних $15$ временных точках:   $$ |y(t)| \leq 0.3, \quad t = T-15, \ldots, T-1.$$

Доступны управления, для которых $$|u(t)| \leq 0.7, \quad t = 0, \ldots, T-1.$$

Горизонт планирования $T=135$, абсолютное значение ошибки ограничено величиной $\varepsilon = 0.02$.
...
На рис. \ref{example1-u} изображена реализация $u^p(\tau)$, $\tau = 0, \ldots, T-1$, оптимальной обратной связи в рассматриваемом процессе. Выходной сигнал (см. рис. \ref{example1-y}) удовлетворяет ограничениям на промежутке от $\tau = 120$ до $\tau=134$.
...
\end{figure}
\begin{samepage}
На рис. \ref{example1-x} изображена реализовавшаяся траектория системы (\ref{example1}). Априорная оценка значения критерия качества, полученная в $\tau = 2$ составила $ 46.3322$. В процессе управления значение критерия качества улучшилось до $42.5599$. Наиболее значительное изменение происходит при $\tau = 2, \ldots, 28$. Этот фрагмент приведен на рис. \ref{example1-J}. 


\section{Пример 2: система четвертого порядка}
...
\end{pmatrix}x(t), \quad t= 0, \ldots, T-1.\nonumber
\end{align}
Снова требуется минимизировать величину $\sum_{t=0}^{T-1}\norm{u(t)}^2$ при следующих ограничениях: 
$$ |y_i(t)| \leq 0.1, \quad t = T-20, \ldots, T-1, \quad i=1,2,$$
$$|u_i(t)| \leq 0.8, \quad t = 0, \ldots, T-1, \quad i=1,2.$$ 
Положим $T=55$, $\varepsilon = 0.02$.
...
\caption{}
\end{subfigure}
\caption{Реализация оптимальной обратной связи (a)--(b)}
\label{fig:example2-u}
\end{figure}
%\medskip
...
\caption{}
\end{subfigure}
\caption{Выходные сигналы (a), изменение критерия качества (b)}
\label{fig:example2}
\end{figure}

Рис. \ref{fig:example2-u}, \ref{fig:example2} иллюстрируют результаты применения предложенной схемы управления. Априорная оценка значения критерия качества, составлявшая $32.183$,  была улучшена до $21.857$. Начиная с момента $\tau = 29$ изменения не  существенны.