...
\begin{theorem}
Пусть в момент $\tau$ для неточной траектории $\{u^p_\tau, \tilde y^p_\tau\}$ разрешима каждая из задач (\ref{estimation}). Далее, пусть разрешима соответствующая задача (\ref{control}) с параметром $\chi(\tau)$, поставленным решениями задач (\ref{estimation}). Тогда в момент $\tau+1$ для траектории $\{u^p_\tau, u^*(\tau|\tau), \tilde y^p_\tau, \tilde y^p(\tau)\}$ эти задачи также разрешимы. Более того, $$\sum\limits_{t=\tau+1}^{T-1}\norm{u^*(t|\tau+1, u^p_{\tau+1}, \tilde y^p_{\tau+1})}^2 \leq \sum\limits_{t=\tau+1}^{T-1}\norm{u^*(t|\tau, u^p_\tau, \tilde y^p_\tau)}^2$$
\end{theorem}

\begin{proof}
На протяжении всего доказательства будут фигурировать четыре задачи: оптимального наблюдения (\ref{estimation-lp-tau}) и управления (\ref{control-qp-tau}) в момент $\tau$, оптимального наблюдения (\ref{estimation-lp-tau+1}) и управления (\ref{control-qp-tau+1}) в момент $\tau+1$. Докажем, что из разрешимости (\ref{estimation-lp-tau}),(\ref{control-qp-tau}) следует разрешимость (\ref{estimation-lp-tau+1}),(\ref{control-qp-tau+1}). 
\begin{multicols}{2}
\noindent
\begin{gather} 
\chi_i(\tau+1) = \max\limits_{\hat{\alpha}(\tau+1)} G_{(\tau+1) i}Y_{\tau+1}^f\hat{\alpha}(\tau+1), \label{estimation-lp-tau+1}\\
\begin{pmatrix}
U_{\tau}^p \\
u(\tau)\\
U_{\tau+1}^f \\
\end{pmatrix} \hat{\alpha}(\tau+1) = \begin{pmatrix}
u_{\tau}^p \\
u^*(\tau|\tau)\\
0 \\
\end{pmatrix}, \nonumber \\
...
y(\tau) \hat \alpha(\tau+1) \leq
\tilde y^p(\tau) + \varepsilon \mathbbm{1}. \nonumber 
\end{gather}
\columnbreak
\begin{gather}
\chi_i(\tau) = \max\limits_{\hat{\alpha}(\tau)} G_{\tau i}Y_{\tau}^f\hat{\alpha}(\tau), \label{estimation-lp-tau} \\
\begin{pmatrix}
U_{\tau}^p \\
u(\tau)\\
U_{\tau+1}^f \\
...
\noindent
\begin{gather}
\min\limits_{\alpha_0(\tau+1)}\alpha_0(\tau+1)^T{U_{\tau+1}^f}^TU_{\tau+1}^f\alpha_0(\tau+1), \label{control-qp-tau+1}\\
\begin{pmatrix}
U_{\tau}^p \\
u(\tau)\\
Y_{\tau}^p \\
...
0 \\
\end{pmatrix}, \nonumber \\
G_{\tau+1}Y_{\tau+1}^f\alpha_0(\tau+1) \leq g_{\tau+1} - \chi(\tau+1), \nonumber \\
u_{\min}(\tau+1) \leq U_{\tau+1}^f\alpha_0(\tau+1) \leq u_{\max}(\tau+1). \nonumber 
\end{gather}
\columnbreak
\begin{gather}
...
Здесь и далее $u(\tau)=(u^d(\tau), u^d(\tau+1), \ldots, u^d(\tau+T^d-T)),$
${y(\tau)=(y^d(\tau), y^d(\tau+1), \ldots, y^d(\tau+T^d-T)).}$
 
Прежде всего, необходимо доказать разрешимость каждой из задач оптимального наблюдения (\ref{estimation-lp-tau+1}) в момент $\tau+1$. Однако, уже на этом этапе придется задействовать задачу (\ref{control-qp-tau}) оптимального управления в момент $\tau$. Во-первых, будем использовать само решение $\alpha_0^*(\tau)$ этой задачи, которое удовлетворяет
...
0\\
y_0^*(\tau|\tau)\\
\end{pmatrix}, \label{alpha_0-tau} \\
G_{\tau+1}Y_{\tau+1}^f \alpha_0^*(\tau) \leq g_{\tau+1}-\chi(\tau+1). \label{alpha_0-tau-y} 
\end{gather}

Во-вторых, понадобится оптимальное управление $U^f_\tau\alpha_0^*(\tau) = u^*(\tau)$, а точее, его хвост $u_0^c(\tau+1)=(u^*(\tau+1|\tau)^T, \ldots, u^*(T-1|\tau)^T)^T$. Для $u_0^c(\tau+1)$ сконструируем некоторое $\alpha_0^c(\tau+1)$, удовлетворяющее следующему равенству:
...
Согласно Лемме \ref{dd-separation}, решение $\alpha_0^c(\tau+1)$ для (\ref{alpha_0-candidate}) существует для всех возможных $u_0^c(\tau+1)$. 
%и этому $\alpha_0^c(\tau+1)$ соответствует $y^c_0(\tau+1)$ ---  выходная траектория, если система движется из состояния $x(\tau+1)=0$ под воздействием хвоста $u_0^c$ оптимального управления предыдущего шага схемы.

Теперь, располагая $\alpha_0^c(\tau+1)$ и $\alpha_0^*(\tau)$, докажем разрешимость задач оптимального наблюдения. Сдвинем переменную ${\hat \alpha(\tau+1)}$, относительно которой поставлены задачи оптимального наблюдения, на константу, перейдем к новой переменной $\Delta\alpha(\tau+1)$:
$$\hat \alpha(\tau+1) = \alpha_0^*(\tau)-\alpha_0^c(\tau+1)+\Delta\alpha(\tau+1).$$

С учетом того, что для $\alpha_0^*(\tau)$ выполняется (\ref{alpha_0-tau}), а $\alpha_0^c(\tau+1)$ удовлетворяет (\ref{alpha_0-candidate}), из задач для определения $\chi(\tau+1)$, поставленных для $\hat \alpha(\tau+1)$, получаем следующие эквивалентные задачи относительно переменной $\Delta\alpha(\tau+1)$: 
\begin{equation} \label{estimation-delta-alpha}
\chi_i(\tau+1) = G_{(\tau+1)i}Y_{\tau+1}^f(\alpha_0^*(\tau)-\alpha_0^c(\tau+1)) +\max\limits_{\Delta\alpha(\tau+1)} G_{(\tau+1) i}Y_{\tau+1}^f\Delta\alpha(\tau+1),
\end{equation}
...
\tilde y^p(\tau)-y_0^*(\tau|\tau) - \varepsilon \mathbbm{1} \leq y(\tau) \Delta \alpha(t+1)
\leq \tilde y^p(\tau)-y_0^*(\tau|\tau) +  \varepsilon \mathbbm{1}.
\end{equation}

Сравним задачи (\ref{estimation-delta-alpha}) для $\Delta \alpha(\tau + 1)$ и (\ref{estimation-lp-tau}) для $\hat \alpha (\tau)$: на $\Delta \alpha(\tau+1)$ наложено дополнительное ограничение (\ref{estimation-new-constraint}), а целевые функции разнятся на константу. 
...
\chi(\tau+1) \leq G_{\tau+1}Y_{\tau+1}^f(\alpha_0^*(\tau)-\alpha_0^c(\tau+1)) + \chi_{\tau+1}(\tau)
\end{equation}

Итак, разрешимость задач оптимального оценивания доказана. Остается продемонстрировать, что множество допустимых решений $\alpha_0(\tau+1)$ в задаче оптимального управления (\ref{control-qp-tau+1}) непусто --- тогда эта выпуклая задача квадратичного программирования имеет решение. Покажем, что $\alpha_0^c(\tau+1)$ --- допустимое решение. Уже по построению $\alpha_0^c(\tau+1)$ выполнены все условия, кроме условий на выходной сигнал: 
\begin{equation}\label{y_0-c-constraint}
G_{\tau+1} Y_{\tau+1}^f \alpha_0^c(\tau+1) \leq g_{\tau+1}-\chi(\tau+1).
\end{equation}
Воспользовавшись сначала (\ref{chi-bound}), а затем (\ref{alpha_0-tau-y}), получаем
\begin{equation*}
\begin{split}
G_{\tau+1} Y_{\tau+1}^f \alpha_0^c(\tau+1) + \chi(\tau+1) & \leq G_{\tau+1}  Y_{\tau+1}^f \alpha_0^c(\tau+1) + \\
& + G_{\tau+1}Y_{\tau+1}^f(\alpha_0^*(\tau)-\alpha_0^c(\tau+1)) + \chi_{\tau+1}(\tau) = \\
& = G_{\tau+1}Y_{\tau+1}^f\alpha_0^*(\tau) + \chi_{\tau+1}(\tau) \leq g_{\tau+1},
\end{split}
\end{equation*}
что доказывает (\ref{y_0-c-constraint}).
Наконец, поскольку решение 
$\alpha_0^c(\tau+1)$ допустимо, 
\begin{equation*}
\begin{split}
\sum\limits_{t=\tau+1}^{T-1}\norm{u^*(t|\tau+1, u^p_{\tau+1}, \tilde y^p_{\tau+1})}^2 & \leq \alpha_0^c(\tau+1)^T (U_{\tau+1}^f)^T U_{\tau+1}^f \alpha_0^c(\tau+1) = \\
& = \sum\limits_{t=\tau+1}^{T-1}\norm{u^*(t|\tau, u^p_\tau, \tilde y^p_\tau)}^2,
\end{split}
\end{equation*} что завершает доказательство всех пунктов теоремы.
